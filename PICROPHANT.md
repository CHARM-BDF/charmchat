...
- **Budget**: caps the number of claims (`MAX_CLAIMS = 12`) and rounds (`PICROPHANT_MAX_ITERATIONS = 10`), since one call fans out into many tool calls. (Raised from 8: at 8, extraction's "most load-bearing first" ranking kept pruning the foundational mechanism claim — e.g. "the variant causes haploinsufficiency" — so a contradiction had no node to land on. With room for 12 it survives, and the verdict spread surfaces it by contrast without starving grounding.)
